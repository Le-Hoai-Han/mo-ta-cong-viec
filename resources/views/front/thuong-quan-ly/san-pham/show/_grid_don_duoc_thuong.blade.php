<x-simple-card buttonCol="7" labelCol="5" extClass="mt-3" headerClass="bg-dark text-white "> 
    ...
    <x-slot name="button">
        @if(auth()->user()->can('add_quanlysanphams') && !$thuongSanPhamQuanLy->daKhoa())
            ...
            <a href="#" class='btn btn-warning' onclick='capNhatThongTinThuong()'>
    ...
    <table class="table">
        ...
        <tbody>
            @forelse($dsSanPhamThuong as $index=>$sanPham) 
                <tr>
                    <td>{{$index+1}}</td>
            ...
            <tr class="bg-secondary">
                ...
                <th style="text-align:right" class="text-white">{{thuGonSoLe($dsSanPhamThuong->sum('so_tien_tinh_thuong'))}}đ</th>
                <th style="text-align:right;font-weight:bold" class="text-white">{{thuGonSoLe($dsSanPhamThuong->sum('so_tien_tinh_thuong')*$thuongSanPhamQuanLy->tiLeTinhThuongCuaNhanVien())}}đ ({{$thuongSanPhamQuanLy->tiLeTinhThuongCuaNhanVien()*100}}%)</th>
                <!-- <th></th> -->
            </tr>
        </tbody>
    </table>
