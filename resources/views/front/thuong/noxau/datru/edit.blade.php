<?php 
$current = "Cập nhật thông tin thanh toán";
$list = [    
...
<x-dashboard-layout :current="$current" :list="$list">
    <div class="main-div col-xs-12 col-md-6">
        <form action="{{route('no-xau.chi-tiet.update',$chiTietNoXauDaTru)}}" method="POST">
            <x-simple-card buttonCol="7" labelCol="5" extClass="mt-3" headerClass="bg-info text-white "> 
                ...
                <div class="row">
                    <div class="form-group">
                        ...
                        <input type="text" name="ngay_tru_no" id="ngay_tru_no" class="form-control" placeholder="Nhập dạng dd/mm/YYYY" value="{{formatNgayDMY($chiTietNoXauDaTru->ngay_tru_no)}}" />
                        @error('ngay_tru_no')
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="so_tien" id="so_tien" class="form-control" placeholder="Nhập số tiền"  value="{{thuGonSoLe($chiTietNoXauDaTru->so_tien)}}"/>
                        @error('so_tien')
                            <span class="help text-danger"> {{ $message}}</span>
                        @enderror
                    </div>
